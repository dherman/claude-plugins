 ...
 *
 * Provides a `log` tool that:
 * - Appends timestamped messages to session transcript logs
 * - Supports optional detailed messages stored in separate files
 * - Generates unique IDs for detail files
 */
...
function generateDetailsId(): string {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  if (request.params.name === "log") {
    const args = request.params.arguments as unknown as LogArgs;
