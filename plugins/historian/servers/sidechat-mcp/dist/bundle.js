...
function getInboxPath(session, agentId) {
...
async function receiveMessage(args) {
  ...
  server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
      tools: [
        {
          name: "send_message",
          description: "Send a message to an agent's inbox within a session. The session ID must be agreed upon by all coordinating agents. Messages are delivered in FIFO order.",
          inputSchema: {
            type: "object",
            properties: {
              session: {
        ...
        },
        {
          ...
          description: "Receive the next message from an agent's inbox. Blocks until a message arrives or timeout is reached. Returns null if timeout is reached with no message.",
          inputSchema: {
            type: "object",
            properties: {
              session: {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      if (name === "receive_message") {
        const result = await receiveMessage(args);
        return {
          content: [
            {
              type: "text",
